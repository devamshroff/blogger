...
public class Editor extends HttpServlet {
        ...
        throws ServletException, IOException
    {
                ...
                Timestamp current = localDateFormat.format(new Date());
                if (postid<=0)
                {
                    try {
                        cmd1 = "SELECT MAX(postid) AS max_p_id FROM Posts"; 
                        rs = s.executeQuery(cmd1);
                        int max_id = rs.getInt("max_p_id")+1;
                        cmd = "INSERT INTO Posts (username,postid,title,body,modified,created)  VALUES (" + username + "," + max_id + "," + title+","+body+","+current+","+current+")"; 
                        rs = s.executeQuery(cmd);
                ...
                else
                {
                    try {
                        ...
                        cmd = "DELETE FROM Posts WHERE username = '" + username + "' AND postid = '" + postid + "';";
                        rs = s.executeQuery(cmd);
                        cmd = "INSERT INTO Posts (username,postid,title,body,modified,created)  VALUES (" + username + "," + postid + "," + title+","+body+","+current+","+date_created+")"; 
                        rs = s.executeQuery(cmd);
                    }
                    catch (SQLException ex){
                        System.out.println("SQLException caught");
